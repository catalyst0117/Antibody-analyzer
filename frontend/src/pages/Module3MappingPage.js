        ...
        const loadHandoffFiles = async () => {
            try {
                const [positiveResponse, negativeResponse] = await Promise.all([
                    apiClient.get(`/results/${handoffState.resultId}/files/${encodeURIComponent(handoffState.positiveFilename)}`, { responseType: "blob" }),
                    apiClient.get(`/results/${handoffState.resultId}/files/${encodeURIComponent(handoffState.negativeFilename)}`, { responseType: "blob" }),
                ]);
                if (cancelled) {
                    return;
                }
