...
h1{font-size:24px;margin:0 auto 0 0}.control{display:grid;gap:4px;font-size:12px;color:#52606d}.control input{height:36px;border:1px solid #b0bec5;border-radius:7px;padding:0 10px;background:white}
...
const canvas=document.getElementById('canvas'),ctx=canvas.getContext('2d'),tip=document.getElementById('tip'),cutoffEl=document.getElementById('cutoff');
const pad={l:72,r:24,t:24,b:58};let screenX=new Float32Array(points.length),screenY=new Float32Array(points.length),drag=null,hoverFrame=0;
...
cutoffEl.addEventListener('input',draw);document.getElementById('reset').addEventListener('click',reset);new ResizeObserver(resize).observe(canvas.parentElement);resize();
