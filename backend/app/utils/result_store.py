...
class ResultStore:
    ...
    def __init__(self, base_dir: Optional[Path] = None, max_age_seconds: int = 60 * 60):
        env_base = os.getenv("RESULT_ARCHIVE_DIR") if base_dir is None else None

        if base_dir is not None:
            resolved_dir = Path(base_dir)
        elif env_base:
            resolved_dir = Path(env_base).expanduser()
        else:
            resolved_dir = Path(tempfile.mkdtemp(prefix="antibody_results_"))
